// sequential schemes derived from https://github.com/axismaps/colorbrewer/
...
export const sequential_hexes = Object.fromEntries(
  Object.entries(sequential).map(([scheme, ranges]) => [
    scheme,
    Object.fromEntries(
      Object.entries(ranges).map(([amount, rgbs]) => [amount, rgbs.map(toHex)])
    ),
  ])
);
...
function toHex(rgb: string): number {
  let sep = rgb.indexOf(",") > -1 ? "," : " ";
  let rgb_l = rgb.slice(4).split(")")[0].split(sep);

  let r = (+rgb_l[0]).toString(16);
  let g = (+rgb_l[1]).toString(16);
  let b = (+rgb_l[2]).toString(16);
